...
export { AddAsignarCargos };
function AddAsignarCargos({ history }) {
  ...
  const validationSchema = Yup.object().shape({
    ...
    tipoSueldo: Yup.string().required("Tipo Sueldo es requerido"),
  ...
  async function create(data) {
    try {
      await userActions.registrarContratoConSueldo(data);
      history.push("/asignarCargos");
      alertActions.success("Contrato añadido");
    } catch (error) {
      console.error(error);
      alertActions.error(`Error al añadir contrato: ${error.message}`);
    }
  }
  const handleLimpiar = () => {
    reset(aporte);
    ...
    <>
      ...
      {!loading && (
        <Form>
          ...
          <Row>
            <Col>
              <h3 className="mb-3">Apartado de Sueldos</h3>
              <Row>
                <Col>
                  <Form.Group className="mb-3" controlId="formTipoSueldo">
                    <Form.Label>Tipo Sueldo</Form.Label>
                    <select
                      name="tipoSueldo"
                      {...register("tipoSueldo")}
                      className="custom-select mr-sm-2"
