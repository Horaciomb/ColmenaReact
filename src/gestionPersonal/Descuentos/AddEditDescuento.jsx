...
import { DescuentoAtom } from "_state";
...
export { AddEditDescuento };
function AddEditDescuento({ history, match }) {
  const { id } = match.params;
  const mode = { add: !id, edit: !!id };
  const userActions = useUserActions();
  const alertActions = useAlertActions();
  const ingreso = useRecoilValue(DescuentoAtom);
  ...
  useEffect(() => {
    async function getData() {
      const result = await client.query(getTipoNominaQuery(3));
  ...
  useEffect(() => {
    if (mode.edit) {
      userActions.getDescuentoId(id);
    }
    return userActions.resetDescuento;
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  useEffect(() => {
  ...
  async function createAporte(data) {
    console.log(data);
    await userActions.registrarDescuento(data);
    history.push("/gestionarDescuentos");
    alertActions.success("Descuento añadido");
  }
  async function updateAporte(id, data) {
    await userActions.updateDescuento(id, data);
    history.push("/gestionarDescuentos");
    alertActions.success("Descuento actualizado");
  }
  const handleLimpiar = () => {
    reset(ingreso);
    setEmpleadoSeleccionada({
  ...
  return (
    <>
      <h1>{mode.add ? "Agregar Descuentos" : "Editar Descuentos"}</h1>
      {!loading && (
        <Form>
          <Row>
            <Col>
          ...
          <div className="form-group">
            ...
            <Link to="/gestionarDescuentos" className="btn btn-link">
              Cancelar
            </Link>
          </div>
        </Form>
