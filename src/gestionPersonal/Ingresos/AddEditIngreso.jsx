...
import { IngresoAtom } from "_state";
...
function AddEditIngreso({ history, match }) {
  ...
  const ingreso = useRecoilValue(IngresoAtom);
  ...
  useEffect(() => {
    async function getData() {
      const result = await client.query(getTipoNominaQuery(4));
  ...
  useEffect(() => {
    if (mode.edit) {
      userActions.getIngresoId(id).then((rciva) => {});
    }
    return userActions.resetIngreso;
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  useEffect(() => {
  ...
  async function createAporte(data) {
    console.log(data);
    await userActions.registrarIngreso(data);
    history.push("/gestionarIngresos");
    alertActions.success("Ingreso añadido");
  }
  async function updateAporte(id, data) {
    await userActions.updateIngreso(id, data);
    history.push("/gestionarIngresos");
    alertActions.success("Ingreso actualizado");
  }
  const handleLimpiar = () => {
    reset(ingreso);
    setEmpleadoSeleccionada({
  ...
  return (
    <>
      <h1>{mode.add ? "Agregar Ingresos" : "Editar Ingresos"}</h1>
      {!loading && (
        <Form>
          <Row>
            <Col>
          ...
          <div className="form-group">
            ...
            <Link to="/gestionarIngresos" className="btn btn-link">
              Cancelar
            </Link>
          </div>
        </Form>
