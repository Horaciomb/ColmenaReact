...
function AddEditAporteRiesgoComun({ history, match }) {
  const { id } = match.params;
  const mode = { add: !id, edit: !!id };
  const userActions = useUserActions();
  const alertActions = useAlertActions();
  const aporte = useRecoilValue(AporteRiesgoComunAtom);
  ...
  useEffect(() => {
    if (mode.edit) {
      userActions.getAporteRiesgoComunById(id);
    }
    return userActions.resetAporteRiesgoComun;
  ...
  async function createAporte(data) {
    await userActions.registrarAporteRiesgoComun(data);
    history.push("/gestionarAporteRiesgoComun");
    alertActions.success("Porcentaje de Aporte Riesgo Común añadido");
  }
  async function updateAporte(id, data) {
    await userActions.updateAporteRiesgoComun(id, data);
    history.push("/gestionarAporteRiesgoComun");
    alertActions.success("Porcentaje de Aporte Riesgo Común actualizado");
  }
      ...
      {!loading && (
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="form-group">
            ...
            <Link to="/gestionarAporteRiesgoComun" className="btn btn-link">
              Cancelar
            </Link>
          </div>
        </form>
